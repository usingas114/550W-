...
private:
    ...
    atomic<int> currentTrack;
    atomic<bool> isPlaying;
    atomic<bool> isPaused;
    RandomGenerator rng;
    
public:
    MusicPlayer() : currentTrack(0), isPlaying(false), isPaused(false) {
        // 初始化一些默认音乐
        playlist = {
            "量子交响曲 - 未知作曲家",
            "二进制节奏 - 数字艺术家",
    ...
    void play() {
        ...
        isPlaying = true;
        isPaused = false;
        cout << ">> 正在播放: " << playlist[currentTrack] << "\n";
    }
    ...
    void next() {
        ...
        if (isPlaying && !isPaused) {
            cout << ">> 下一首: " << playlist[currentTrack] << "\n";
        }
    }
    ...
    void previous() {
        ...
        if (isPlaying && !isPaused) {
            cout << ">> 上一首: " << playlist[currentTrack] << "\n";
        }
    }
    ...
    void scanDirectory(const string& path) {
        ...
        // 模拟扫描过程
        vector<string> musicExtensions = {".mp3", ".wav", ".flac", ".aac", ".ogg", ".m4a"};
        vector<string> foundFiles;
        
        cout << ">> 开始量子音频分析...\n";
        
        // 模拟找到的文件
        for (int i = 0; i < 8 + rng.getRandom(0, 10); ++i) {
            string artist = "";
            switch(rng.getRandom(0, 4)) {
                case 0: artist = "量子乐队"; break;
                case 1: artist = "光子歌手"; break;
                case 2: artist = "二进制合唱团"; break;
                case 3: artist = "算法大师"; break;
                case 4: artist = "数据流乐团"; break;
            }
            
            string title = "";
            switch(rng.getRandom(0, 5)) {
                case 0: title = "宇宙旋律"; break;
                case 1: title = "数字梦想"; break;
                case 2: title = "光子之舞"; break;
                case 3: title = "量子纠缠"; break;
                case 4: title = "二进制爱情"; break;
                case 5: title = "数据河流"; break;
            }
            
            string extension = musicExtensions[rng.getRandom(0, musicExtensions.size()-1)];
            string filename = title + extension;
            foundFiles.push_back(artist + " - " + title);
        }
        
        // 显示扫描结果
        cout << ">> 发现 " << foundFiles.size() << " 个音频文件:\n";
        ...
        cout << ">> 扫描完成，播放列表现在有 " << playlist.size() << " 首曲目\n\n";
    }
    
    // 播放系统音频文件（Windows版本）
    bool playSystemAudio(const string& filePath) {
        cout << ">> 尝试播放系统音频文件: " << filePath << "\n";
        
        if (!fileExists(filePath)) {
            cout << ">> [错误] 文件不存在: " << filePath << "\n";
            return false;
        }
        ...
        vector<string> supportedFormats = {".mp3", ".wav", ".flac", ".aac", ".ogg", ".m4a", ".mid", ".midi"};
        ...
        if (mciSendStringA(command.c_str(), NULL, 0, NULL) == 0) {
            ...
            cout << ">> 正在使用Windows媒体播放器播放: " << filePath << "\n";
            cout << ">> 输入 'music stopfile' 停止播放\n";
            return true;
        } else {
            // 如果mci失败，尝试使用系统默认程序
            string sysCommand = "start \"\" \"" + filePath + "\"";
            int result = system(sysCommand.c_str());
            if (result == 0) {
                cout << ">> 正在使用系统默认播放器播放: " << filePath << "\n";
                return true;
            }
        }
    ...
    string getStatus() const {
        ...
        return "播放中: " + playlist[currentTrack];
    }
};
...
private:
    ...
    // 音乐播放器
    unique_ptr<MusicPlayer> musicPlayer;
    ...
    void initializeNewFeatures() {
        ...
        // 初始化音乐播放器
        musicPlayer.reset(new MusicPlayer());
    }
    ...
    void initializeCommandMap() {
        commandMap = {
            ...
            {"scan", [this](const string& args) { scanDisk(args); }},
    ...
    void musicControl(const string& args) {
        ...
        if (args.empty()) {
            cout << "\n[音乐播放器] 可用命令:\n";
            cout << "music play           - 播放音乐\n";
            cout << "music pause          - 暂停/继续\n";
            cout << "music stop           - 停止播放\n";
            cout << "music next           - 下一首\n";
            cout << "music prev           - 上一首\n";
            cout << "music list           - 显示播放列表\n";
            cout << "music add [名]       - 添加曲目\n";
            cout << "music status         - 播放状态\n";
            cout << "music playfile [路径] - 播放系统音频文件\n";
            cout << "music stopfile       - 停止系统音频播放\n";
            return;
        }
        ...
        if (subcmd == "play") {
            musicPlayer->play();
            logMessage("音乐播放器: 开始播放");
        }
        else if (subcmd == "pause") {
            musicPlayer->pause();
            logMessage("音乐播放器: 暂停/继续");
        }
        else if (subcmd == "stop") {
            musicPlayer->stop();
            logMessage("音乐播放器: 停止播放");
        }
        else if (subcmd == "next") {
            musicPlayer->next();
            logMessage("音乐播放器: 下一首");
        }
        else if (subcmd == "prev") {
            musicPlayer->previous();
            logMessage("音乐播放器: 上一首");
        }
        else if (subcmd == "list") {
            musicPlayer->showPlaylist();
            logMessage("音乐播放器: 显示播放列表");
        }
        else if (subcmd == "add") {
            string track;
            getline(iss, track);
            if (!track.empty() && track[0] == ' ') {
                track = track.substr(1);
            }
            if (!track.empty()) {
                musicPlayer->addTrack(track);
                logMessage("音乐播放器: 添加曲目 - " + track);
            } else {
                cout << ">> 错误: 请输入曲目名称\n";
            }
        }
        else if (subcmd == "status") {
            cout << "\n[音乐状态] " << musicPlayer->getStatus() << "\n\n";
            logMessage("音乐播放器: 状态查询");
        }
        else if (subcmd == "playfile") {
            string filePath;
            ...
            if (!filePath.empty()) {
                if (musicPlayer->playSystemAudio(filePath)) {
                    logMessage("音乐播放器: 播放系统文件 - " + filePath);
                } else {
                    logMessage("音乐播放器: 播放系统文件失败 - " + filePath, "WARNING");
                }
            } else {
                cout << ">> 错误: 请输入文件路径\n";
            }
        }
        else if (subcmd == "stopfile") {
            musicPlayer->stopSystemAudio();
            logMessage("音乐播放器: 停止系统音频播放");
        }
        else {
            cout << ">> 未知音乐命令\n";
        }
    ...
    void playSystemBeep() {
        if (musicPlayer) {
            musicPlayer->playSystemBeep();
            logMessage("播放系统提示音");
        }
    }
    
    // 磁盘扫描功能
    void scanDisk(const string& path) {
        string scanPath = path;
        if (scanPath.empty()) {
            cout << "\n[磁盘扫描] 请输入要扫描的路径: ";
            getline(cin, scanPath);
        }
        ...
        cout << "\n[量子磁盘扫描] 开始扫描: " << scanPath << "\n";
        logMessage("开始磁盘扫描: " + scanPath);
        ...
        vector<string> fileTypes = {
            "文档文件", "图像文件", "音频文件", "视频文件", 
            "程序文件", "数据库文件", "配置文件", "日志文件"
        };
        
        vector<string> fileExtensions = {
            ".doc", ".pdf", ".txt", ".jpg", ".png", ".mp3", ".mp4", 
            ".exe", ".dll", ".db", ".cfg", ".log", ".dat", ".bin"
        };
        
        cout << ">> 量子扫描进行中:\n";
        
        int totalFiles = 200 + rng.getRandom(0, 800);
        int scannedFiles = 0;
        
        while (scannedFiles < totalFiles) {
            int batch = min(50 + rng.getRandom(0, 30), totalFiles - scannedFiles);
            
            for (int i = 0; i < batch; ++i) {
                string fileType = fileTypes[rng.getRandom(0, fileTypes.size()-1)];
                string ext = fileExtensions[rng.getRandom(0, fileExtensions.size()-1)];
                string fileName = "file_" + to_string(scannedFiles + i + 1) + ext;
                
                if (rng.getRandom() < 5) { // 5%概率发现特殊文件
                    cout << "   ! 发现特殊文件: " << fileName << " [量子加密]\n";
                }
            }
            ...
            cout << ">> 扫描进度: [" << string(progress/5, '#') << string(20 - progress/5, '.') << "] " << progress << "%\n";
        ...
        // 扫描结果
        cout << "\n[扫描完成]\n";
        cout << ">> 总文件数: " << totalFiles << "\n";
        cout << ">> 可疑文件: " << rng.getRandom(0, 5) << "\n";
        cout << ">> 加密文件: " << rng.getRandom(0, 3) << "\n";
        cout << ">> 系统完整性: " << (95 + rng.getRandom(0, 5)) << "%\n\n";
        
        logMessage("磁盘扫描完成: " + scanPath + ", 文件数: " + to_string(totalFiles));
        ...
        if (answer == "y" || answer == "Y") {
            if (musicPlayer) {
                musicPlayer->scanDirectory(scanPath);
            }
        }
    }
    ...
    void showStatus() {
        ...
        if (musicPlayer) {
            cout << "音乐状态: " << musicPlayer->getStatus() << endl;
        }
